...
router.get("/guest/stats", (req, res) => {
	try {
		const guestCookie = req.cookies?.dr_guest;
		if (!guestCookie) return res.status(404).json({ message: "Guest not found" });
		const guestData = guestSessions.get(guestCookie);
		if (!guestData) return res.status(404).json({ message: "Session expired" });
...
router.get("/guest/stats-volume", (req, res) => {
	try {
		...
		const getStats = (days) => {
			...
			const plus = filtered.filter((h) => h.change > 0).reduce((acc, curr) => acc + curr.change, 0);
			const minus = filtered.filter((h) => h.change < 0).reduce((acc, curr) => acc + curr.change, 0);
			return { plus, minus, total: plus + minus };
		};
		res.json({ today: getStats(1), week: getStats(7), allTime: guestData.stats || { resilience: 50, stabilityDays: 0 }, history });
	} catch (err) {
		res.status(500).json({ message: err.message });
	}
});
...
router.post("/guest/update-resilience", (req, res) => {
	try {
		...
		if (itemId && type) {
			if (['material', 'complete_material', 'material_view'].includes(type)) {
				if (!guestData.completedMaterials) guestData.completedMaterials = [];
				if (!guestData.completedMaterials.includes(itemId)) guestData.completedMaterials.push(itemId);
			} else if (['scenario', 'complete_scenario', 'complete_exercise'].includes(type)) {
		...
		if (calculatedChange < 0) finalChange = Math.min(-1, Math.round(finalChange));
		else if (calculatedChange > 0) finalChange = Math.max(1, Math.round(finalChange));
		else finalChange = 0;
		...
		if (!metadata.hidden) {
			guestData.history.unshift({ activityType: type, activityName: name, change: finalChange, newScore: currentRes, date: new Date() });
		}
		if (guestData.history.length > 50) guestData.history = guestData.history.slice(0, 50);

		guestSessions.set(guestCookie, guestData);
		res.cookie("dr_guest", guestCookie, guestCookieOptions()).json(guestData);
	} catch (err) {
...
router.post("/guest/diagnostic", (req, res) => {
	try {
		...
		const multiplier = score < 50
			? Number((0.1 + (score / 50) * 0.4).toFixed(2))
			: Number((1.1 + ((score - 50) / 50) * 0.4).toFixed(2));

		const updatedData = {
			...currentData,
			stats: { ...(currentData.stats || {}), resilienceMultiplier: multiplier },
			diagnostic: { answers, completedAt: new Date() }
		};
		if (!updatedData.history) updatedData.history = [];
		updatedData.history.unshift({ activityType: 'diagnostic', activityName: 'Діагностика стану', change: 0, newScore: updatedData.stats.resilience || 50, date: new Date() });

		const saved = { ...updatedData, history: (updatedData.history || []).slice(0, 50) };
		guestSessions.set(guestCookie, saved);
		res.cookie("dr_guest", guestCookie, guestCookieOptions()).json({ success: true, score, multiplier, stats: updatedData.stats });
	} catch (err) {
		res.status(500).json({ message: err.message });
	}
});
